...
regular = L1L2(l1=1e-5, l2=1e-4)
model = Sequential()
model.add(Embedding(input_dim=vocabulary, output_dim=64, input_length=MAX_LENGTH))
model.add(SimpleRNN(64, activation="relu", dropout=0.3, recurrent_dropout=0.3, return_sequences=True,
                    kernel_regularizer=regular, bias_regularizer=regular, activity_regularizer=regular,
                    recurrent_regularizer=regular))
